...
/// Fitur kamera, battery, WebView, WindowManager hanya tersedia di Android
bool get hasMobileFeatures => isAndroid;
    ...
    try {
      ...
      if (tokenInput == serverToken) {
        if (mounted) {
          ...
          if (isNative) {
            // Cek Device Admin sebelum masuk ujian (Android only)
            if (isAndroid && _exam!.antiCurang) {
            ...
            // Google Form / Link: cek Device Admin dulu jika antiCurang aktif
            if (isAndroid && _exam!.antiCurang) {
              final isAdmin = await KioskService.isAdminActive();
              if (!isAdmin) {
                if (!mounted) return;
